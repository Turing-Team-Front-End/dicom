...
<template>
  <div class="dicom-main">
      ...
      <div class="m-left">
        ...
        <div :class="['t-tools']">
          <div class="s-title">
            <el-upload class="upload-demo" ref="upload" action="https://jsonplaceholder.typicode.com/posts/"
              :on-change="handlePreview" :auto-upload="false" :show-file-list="false">
          ...
          <div class="s-volume">
            ...
            <el-input-number v-model="num" controls-position="right" @change="changeVolume" :min="1" :max="timeSlices"
              size="small" :disabled="timeSlices == 1"></el-input-number>
          </div>
          <!-- 其他工具 -->
          <div v-for="(item, index) in toolList" :key="item.iconfont" :class="['s-other', item.status ? 's-select' : '']"
            :title="item.title" @click="clickTools(item, index)">
          ...
          <div :class="['s-other t-dialog2', colorStatus ? 's-select' : '']" :title="'colorMap'">
            ...
            <div class="r-tools">
              <div v-for="(color, index) in colorlist" :key="color.id"
                :class="['s-color', color.status ? 's-select' : '']" @click="colorMap(color.id, index)" :title="color.id">
                <i :class="['color-bar' + color.id]"></i><span> {{ color.name }}</span>
              </div>
            </div>
          </div>
          <!-- 视图 -->
          <div :class="['s-other t-dialog3', viewStatus ? 's-select' : '']" title="视图">
            <i class="t-active iconfont iconimage-view"></i>
            <div class="r-tools">
              <div v-for="(meas, index) in viewList" :key="meas.iconfont"
                :class="['s-color', meas.status ? 's-select' : '']" @click="changeView(meas, index)" :title="meas.title">
        ...
        <div class="t-main">
          <div class="n-list" v-show="tabStatus">
            <!-- 图像评估 -->
            <div class="t-image">
              <div class="e-card" v-for="(item, index) in imageMark" :key="item.id"
                @mouseenter="enterOrLeaveTool(item, true)" @mouseleave="enterOrLeaveTool(item, false)"
                v-show="imageMark.length != 0">
                <div class="e-title">
                  <span class="e-e">
                    <el-input size="mini" v-model="item.title" maxlength="20" :placeholder="'#-' + index"
                      @blur="blurTitle(item, index)">
                    </el-input>
                  </span>
                  <span class="e-icon">
                    <i class="el-icon-close" @click.stop="deleteTool(item, index)" title="删除图像评估"></i>
                  </span>
                </div>
                <div class="e-result">
                  {{ item.value }}
          ...
          <div class="n-image" :style="{
            ...
            <div id="nifti-imagex" v-show="niftiNum == 1 || niftiNum == 4"
              :class="['e-x', selectView == 'nifti-imagex' ? 'n-active' : '']" @click="clickView('nifti-imagex')"></div>
            <div id="nifti-imagey" v-show="niftiNum == 2 || niftiNum == 4"
              :class="['e-y', selectView == 'nifti-imagey' ? 'n-active' : '']" @click="clickView('nifti-imagey')"></div>
            <div id="nifti-imagez" v-show="niftiNum == 3 || niftiNum == 4"
              :class="['e-z', selectView == 'nifti-imagez' ? 'n-active' : '']" @click="clickView('nifti-imagez')"></div>
          </div>
    ...
    <!--  -->
    <el-dialog :title="'Slices，共' + spliceList.length + '个（' + splicePrecent + '%）'" :visible="visable" width="1060px"
      :before-close="handleClose" class="dialog" top="7vh" :close-on-press-escape="false" :close-on-click-modal="false">
...
/* 辅助线 */
let synchronizer = new cornerstoneToolsCore.Synchronizer(
  'cornerstonenewimage',
  cornerstoneToolsCore.updateImageSynchronizer
);
...
export default {
  ...
  data() {
    return {
      ...
      /* 在线影像数据地址：解决本地文件跨域 */
      url: '/bold_2x2x2_s4_tr1500_P2A_195_08.nii.gz',
  ...
  methods: {
    ...
    mouseUpHandler(e) {
      ...
      if (
        ...
        e.composedPath()[0].localName === 'input'
      ) {
        return;
      }
      let that = this;
      ...
      list.forEach((item) => {
        if (that.imageMark.length === 0 && e.composedPath()[1].className !== 't-main') {
    ...
    unFlodPanel(val) {
      if (this.percentage === 100 && val) {
        ...
        const {
          toolState,
        } = cornerstoneToolMain.globalImageIdSpecificToolStateManager;
    ...
    mouseMoveHandler(e) {
      ...
      if (
        ...
        e.composedPath()[1].id === 'nifti-imagex'
      ) {
        ...
        e.composedPath()[1].id === 'nifti-imagey'
      ) {
        ...
        e.composedPath()[1].id === 'nifti-imagez'
      ) {
     ...
     */
    handleScroll(e) {
      if (e && e.composedPath()[0].className === 'w-main') {
    ...
    getImageIndex(e) {
      let id =
        e.composedPath()[1].id === this.element.id ? this.element.id : e.composedPath()[1].id;
    ...
    clickTools(val, index) {
      ...
      if (tool === 'play') {
        ...
        data.forEach((item) => {
          ...
          cornerstoneToolsCore.stackPrefetch.enable(ele);
        });
      } else {
        this.clearCross(data, false);
        const toolName = cornerstoneToolsCore[`${tool}Tool`];
    ...
    clickView(val) {
      ...
      if (this.selectView && val !== this.selectView) {
        ...
        // cornerstoneToolsCore.stackScrollWheel.disable(ele);
      }
      this.selectView = val;
      /* 点击切换视图 */
      this.element = element;
      // cornerstoneToolsCore.stackScrollWheel.activate(element);
      this.currentThanLayers =
    ...
    clearCross(data, val2) {
      data.forEach((item) => {
        ...
        cornerstoneToolsCore.stackPrefetch.disable(ele);
        setTimeout(() => {
          if (val2 !== false) {
            cornerstone.reset(ele);
          }
    ...
    loadAndViewImage(imageId, val) {
      ...
      cornerstone.loadAndCacheImage(imageIdObject.url).then(function (image) {
        ...
        /* 工具添加 */
        cornerstoneToolsCore.addStackStateManager(element, ['stack']);
        cornerstoneToolsCore.addToolState(element, 'stack', stack);
        // cornerstoneToolsCore.orientationMarkersTools.enable(element);
